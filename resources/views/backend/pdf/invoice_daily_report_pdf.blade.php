	  ...
	  <div class="row">
		...
		<div class="col-12">
		  <div class="page-header">
			<h2 class="d-inline"><span class="font-size-30">Daily Invoice Report</span></h2>
		...
		<div class="col-12 table-responsive">
		  <table class="table table-bordered">
			...
			@php
				$invoice_sum = 0;
			@endphp
			@foreach($invoice_daily as $row)
			<tr>
			  <td>1</td>
			  <td>{{ @$row->payment->customer->name }}</td>
			  <td>{{ @$row->invoice_no }}</td>
			  <td class="text-right">{{ date('Y-m-d', strtotime(@$row->date)) }}</td>
			  <td class="text-right">{{ @$row->description }}</td>
			  <td class="text-right">${{ @$row->payment->total_amount }}</td>
			</tr>
			@php
				$invoice_sum += @$row->payment->total_amount;
			@endphp
			@endforeach
			</tbody>
		  </table>
	  ...
	  <div class="row">
		<div class="col-12 text-right">
			<div class="total-payment">
				<h3><b>Grand Total :</b> ${{ $invoice_sum }}</h3>
			</div>
